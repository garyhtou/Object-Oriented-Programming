...
//     state
// - P4. May 12th, 2022: Any gridFlea may jump outside grid boundaries, at most
//     once, and the jump cannot be more than z squares away from the boundary.
...
void GridFlea::move(int p) {
    ...
    // Set up pointer to position of current axis and boundary
    int *dirPos;
    int dirBound;
    if (direction == X) {
        dirPos = &x;
    ...
    if (abs(*dirPos) > dirBound - abs(amount)) {
        state = Dead;
    }
    if (jumping && (abs(*dirPos) + abs(amount) >= dirBound + z)) {
        throw invalid_argument(
                "Can not jump more than z squares (" + to_string(z) + ") away");
    }
    (*dirPos) += amount;
